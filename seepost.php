<?php include('include/header.php') ?>
  ...
  <section id="contentSection">
    ...
    <?php
      if(!isset($_GET['search_id']) || $_GET['search_id'] == NULL){
         echo "<script> window.location = '404.php'; </script>";
      }
      else{
          $post_id = $_GET['search_id'];
      }
     ?>
   ...
   if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($email) || empty($comment)) {
      ...
      $i = 0;
       $sql = "INSERT INTO 
            `tbl_comment` (`id`, `username`, `email`, `comment`, `date`, `post_id`,)
           VALUES (NULL, '$username', '$email', '$comment', CURRENT_TIMESTAMP,'$post_id')";
